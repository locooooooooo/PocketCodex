...
fn agent_result_chat_text(result: AgentResult) -> String {
    let project = if result.project.trim().is_empty() {
        "agent"
    } else {
        result.project.trim()
    };
    let status = match result
        .status
        .enum_value_or(AgentResultStatus::AGENT_RESULT_STATUS_FAILED)
    {
        AgentResultStatus::AGENT_RESULT_STATUS_STARTED => "started",
        AgentResultStatus::AGENT_RESULT_STATUS_NEEDS_CONFIRMATION => "needs confirmation",
        AgentResultStatus::AGENT_RESULT_STATUS_RUNNING => "running",
        AgentResultStatus::AGENT_RESULT_STATUS_DONE => "done",
        AgentResultStatus::AGENT_RESULT_STATUS_CANCELLED => "cancelled",
        AgentResultStatus::AGENT_RESULT_STATUS_FAILED => "failed",
    };
    let mut text = format!("[Agent:{project}] {status}");
    if !result.request_id.trim().is_empty() {
        text.push_str(&format!(": {}", result.request_id.trim()));
    }
...
fn agent_result_status_text(result: &AgentResult) -> &'static str {
    match result
        .status
        .enum_value_or(AgentResultStatus::AGENT_RESULT_STATUS_FAILED)
    {
        AgentResultStatus::AGENT_RESULT_STATUS_STARTED => "started",
        AgentResultStatus::AGENT_RESULT_STATUS_NEEDS_CONFIRMATION => "needs_confirmation",
        AgentResultStatus::AGENT_RESULT_STATUS_RUNNING => "running",
        AgentResultStatus::AGENT_RESULT_STATUS_DONE => "done",
        AgentResultStatus::AGENT_RESULT_STATUS_CANCELLED => "cancelled",
        AgentResultStatus::AGENT_RESULT_STATUS_FAILED => "failed",
        ...
        if let Ok(msg_in) = Message::parse_from_bytes(&data) {
            match msg_in.union {
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::AgentResult(result)) => {
                        #[cfg(feature = "flutter")]
                        self.handler.on_agent_result(
                            result.request_id.clone(),
                            result.project.clone(),
                            agent_result_status_text(&result).to_owned(),
                            result.text.clone(),
                            result.token.clone(),
                            result.detail_json.clone(),
                        );
                        #[cfg(not(feature = "flutter"))]
                        self.handler.new_message(agent_result_chat_text(result));
                    }
