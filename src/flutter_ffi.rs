...
pub fn session_send_agent_command(
    session_id: SessionID,
    request_id: String,
    project: String,
    prompt: String,
    ...
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        log::info!(
            "session_send_agent_command session={} request={} project={} mode={} prompt_len={}",
            session_id,
            request_id,
            project,
            mode,
        ...
        session.send_agent_command(request_id, project, prompt, mode, require_confirmation);
    } else {
        log::warn!(
            "session_send_agent_command dropped, session not found: {} request={}",
            session_id,
            request_id
        );
    }
...
pub fn session_send_agent_cancel(session_id: SessionID, request_id: String, token: String) {
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        session.send_agent_cancel(request_id, token);
    }
}
