...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn send_agent_command(
        ...
        let mode = match mode.as_str() {
            ...
            "status" => AgentCommandMode::AGENT_COMMAND_MODE_STATUS,
            "cancel" => AgentCommandMode::AGENT_COMMAND_MODE_CANCEL,
            _ => AgentCommandMode::AGENT_COMMAND_MODE_READ_ONLY,
        };
        let mut misc = Misc::new();
        misc.set_agent_command(AgentCommand {
            request_id,
            project,
            prompt,
            mode: mode.into(),
    ...
    pub fn send_agent_cancel(&self, request_id: String, token: String) {
        let mut misc = Misc::new();
        misc.set_agent_cancel(AgentCancel {
            request_id,
            token,
            ..Default::default()
        });
...
pub trait InvokeUiSession: Send + Sync + Clone + 'static + Sized + Default {
    ...
    fn on_agent_result(
        ...
        let _ = (request_id, project, status, text, token, detail_json);
    }
