...
fn heartbeat_url() -> String {
    ...
    if api_server.is_empty() && !custom_server.is_empty() {
        log::debug!(
            "Skip hbbs_http sync for self-hosted rendezvous server without explicit api-server: {}",
            custom_server
        );
        return "".to_owned();
    }
    let url = crate::common::get_api_server(api_server, custom_server);
