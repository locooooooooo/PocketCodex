...
#[cfg(test)]
mod agent_chat_route_tests {
    use super::parse_agent_chat_route;

    #[test]
    fn parses_basic_agent_chat_prompt() {
        let route = parse_agent_chat_route("rustdesk analyze startup");
        assert_eq!(route.project, "rustdesk");
        assert_eq!(route.prompt, "analyze startup");
        assert_eq!(route.executor, None);
        assert_eq!(route.profile, None);
        assert_eq!(route.session, None);
        assert_eq!(route.resume_last, None);
    }

    #[test]
    fn parses_session_and_profile_options_before_prompt() {
        let route = parse_agent_chat_route("rustdesk profile=default session=@last continue task");
        assert_eq!(route.project, "rustdesk");
        assert_eq!(route.profile.as_deref(), Some("default"));
        assert_eq!(route.session, None);
        assert_eq!(route.resume_last, Some(true));
        assert_eq!(route.prompt, "continue task");
    }

    #[test]
    fn treats_unknown_key_value_as_prompt_start() {
        let route = parse_agent_chat_route("rustdesk foo=bar continue task");
        assert_eq!(route.project, "rustdesk");
        assert_eq!(route.prompt, "foo=bar continue task");
        assert_eq!(route.executor, None);
    }
}
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn agent_chat_message(text: String) -> Message {
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn should_send_agent_started(prompt: &str) -> bool {
    ...
    !matches!(
        action,
        "list_sessions" | "get_session" | "page_session" | "list_skills"
    )
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn agent_result_status(status: &str) -> AgentResultStatus {
    match status {
        "started" => AgentResultStatus::AGENT_RESULT_STATUS_STARTED,
        "needs_confirmation" => AgentResultStatus::AGENT_RESULT_STATUS_NEEDS_CONFIRMATION,
        "running" => AgentResultStatus::AGENT_RESULT_STATUS_RUNNING,
        "done" => AgentResultStatus::AGENT_RESULT_STATUS_DONE,
        "cancelled" => AgentResultStatus::AGENT_RESULT_STATUS_CANCELLED,
        _ => AgentResultStatus::AGENT_RESULT_STATUS_FAILED,
    }
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn agent_result_message(
    request_id: String,
    project: String,
    status: &str,
    text: String,
    ...
    let mut misc = Misc::new();
    misc.set_agent_result(AgentResult {
        request_id,
        project,
        status: agent_result_status(status).into(),
        text,
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn agent_response_summary(response: &crate::agent_bridge::AgentResponse) -> String {
    let project = &response.project;
    match response.status.as_str() {
        "started" => format!("[Agent:{project}] started: {}", response.text),
        "running" => format!("[Agent:{project}] running:\n{}", response.text),
        "done" => format!("[Agent:{project}] done:\n{}", response.text),
        "needs_confirmation" => {
            let mut text = format!("[Agent:{project}] needs confirmation:\n{}", response.text);
            if let Some(token) = response.token.as_deref().filter(|token| !token.is_empty()) {
                text.push_str(&format!("\n\nToken: {token}"));
            }
            text
        }
        "cancelled" => format!("[Agent:{project}] cancelled:\n{}", response.text),
        _ => format!("[Agent:{project}] failed:\n{}", response.text),
    }
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn agent_confirmation_message(
    response: &crate::agent_bridge::AgentResponse,
    token: &str,
) -> Message {
    let text = format!(
        "Project: {}\nRequest: {}\nToken: {}\n\n{}",
        response.project, response.request_id, token, response.text
    );
    let mut msg = Message::new();
    msg.set_message_box(MessageBox {
        msgtype: "agent-confirm".to_owned(),
        title: "Codex Agent Confirmation".to_owned(),
        text,
        link: "".to_owned(),
        ..Default::default()
    });
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn send_agent_chat(mut inner: ConnInner, text: String) {
    inner.send(agent_chat_message(text).into());
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn send_agent_result(mut inner: ConnInner, response: &crate::agent_bridge::AgentResponse) {
    log::info!(
        "#{} agent result sent request={} project={} status={} detail_len={}",
        inner.id(),
        response.request_id,
        response.project,
        response.status,
        response.detail_json.as_ref().map_or(0, String::len)
    );
    inner.send(
        agent_result_message(
            ...
            response.detail_json.clone().unwrap_or_default(),
        )
        .into(),
    );
}
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn send_agent_response(mut inner: ConnInner, response: crate::agent_bridge::AgentResponse) {
    inner.send(agent_chat_message(agent_response_summary(&response)).into());
    if response.status == "needs_confirmation" {
        if let Some(token) = response.token.as_deref().filter(|token| !token.is_empty()) {
            inner.send(agent_confirmation_message(&response, token).into());
        }
    }
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn task_snapshot_detail_json_for_response(
    response: &crate::agent_bridge::AgentResponse,
    task_override: Option<&crate::agent_bridge::AgentTaskInfo>,
) -> Option<String> {
    if !matches!(
        response.status.as_str(),
        "running" | "done" | "failed" | "cancelled" | "needs_confirmation"
    ) {
        return None;
    }
    if let Some(task) = task_override {
        return Some(crate::agent_bridge::task_snapshot_detail(task).to_string());
    }
    crate::agent_bridge::send_task_status_request(&response.request_id)
        .ok()
        .map(|task| crate::agent_bridge::task_snapshot_detail(&task).to_string())
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn send_agent_protocol_response(inner: ConnInner, response: crate::agent_bridge::AgentResponse) {
    send_agent_protocol_response_with_task(inner, response, None);
}

#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn send_agent_protocol_response_with_task(
    mut inner: ConnInner,
    response: crate::agent_bridge::AgentResponse,
    task_override: Option<&crate::agent_bridge::AgentTaskInfo>,
) {
    send_agent_result(inner.clone(), &response);
    if let Some(detail_json) = task_snapshot_detail_json_for_response(&response, task_override) {
        send_agent_result(
            inner.clone(),
            &crate::agent_bridge::AgentResponse {
...
fn send_agent_failure(
    ...
    let response = crate::agent_bridge::AgentResponse {
        request_id,
        project,
    ...
    if chat_compat && response.project.is_empty() {
        send_agent_chat(inner, format!("{fallback_prefix}\n{}", response.text));
    } else if chat_compat {
        send_agent_response(inner, response);
    } else {
        send_agent_protocol_response(inner, response);
    }
}
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
const AGENT_CONFIRM_PREFIX: &str = "/agent-confirm ";
#[cfg(not(any(target_os = "android", target_os = "ios")))]
const AGENT_CANCEL_PREFIX: &str = "/agent-cancel ";

#[cfg(not(any(target_os = "android", target_os = "ios")))]
#[derive(Default)]
struct AgentChatRoute {
    project: String,
    prompt: String,
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
fn parse_agent_chat_route(text: &str) -> AgentChatRoute {
    let mut tokens = text.trim().split_whitespace();
    let mut route = AgentChatRoute {
        project: tokens.next().unwrap_or_default().to_owned(),
        ..Default::default()
    };
    ...
    for token in tokens {
        if !prompt_started {
            if let Some((key, value)) = token.split_once('=') {
                ...
                if !value.is_empty() {
                    if key.eq_ignore_ascii_case("executor") {
                        route.executor = Some(value.to_owned());
                        continue;
                    }
                    if key.eq_ignore_ascii_case("profile") {
                        route.profile = Some(value.to_owned());
                        continue;
                    }
                    if key.eq_ignore_ascii_case("session")
                        || key.eq_ignore_ascii_case("thread")
                        || key.eq_ignore_ascii_case("dialog")
                    {
                        if value.eq_ignore_ascii_case("@last") || value.eq_ignore_ascii_case("last")
                        {
                            route.session = None;
                            route.resume_last = Some(true);
                        } else {
                            route.session = Some(value.to_owned());
                            route.resume_last = Some(false);
                        }
                        continue;
                    }
                }
        ...
        prompt_tokens.push(token.to_owned());
    }
    route.prompt = prompt_tokens.join(" ");
    route
}
...
impl Connection {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn try_handle_agent_chat(&mut self, text: &str) -> bool {
        if let Some(rest) = text.strip_prefix(AGENT_CONFIRM_PREFIX) {
            let token = rest.trim().to_owned();
            self.spawn_agent_confirm(token);
            return true;
        }
        if let Some(rest) = text.strip_prefix(AGENT_CANCEL_PREFIX) {
            let id_or_token = rest.trim().to_owned();
            self.spawn_agent_cancel(id_or_token);
            return true;
        }
        let Some(rest) = text.strip_prefix(AGENT_PREFIX) else {
            return false;
        };
        let route = parse_agent_chat_route(rest);
        ...
        std::thread::spawn(move || {
            if project.is_empty() || prompt.is_empty() {
                send_agent_failure(
                    ...
                    request_id,
                    project,
                    "Usage: /agent <project_id> [session=@last|session=<id>] [profile=<name>] [executor=<name>] <prompt>".to_owned(),
                    "[Agent] failed:".to_owned(),
                    chat_compat,
                );
                return;
            }
            let started = crate::agent_bridge::AgentResponse {
            ...
            if should_send_agent_started(&prompt) {
                if chat_compat {
                    send_agent_chat(
                        inner.clone(),
                        format!("[Agent:{project}] started: {request_id}"),
                    );
                } else {
                    send_agent_result(inner.clone(), &started);
                }
            }
            let req = crate::agent_bridge::AgentRunRequest {
                request_id: request_id.clone(),
                project: project.clone(),
                prompt,
            ...
            match crate::agent_bridge::send_run_request(&req) {
                Ok(response) => {
                    if chat_compat {
                        send_agent_response(inner, response);
                    } else {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn spawn_agent_confirm(&mut self, token: String) {
        let inner = self.inner.clone();
        std::thread::spawn(move || {
            if token.is_empty() {
                send_agent_chat(
                    inner,
                    "[Agent] failed:\nUsage: /agent-confirm <token>".to_owned(),
                );
                return;
            }
            send_agent_chat(inner.clone(), "[Agent] confirmation received".to_owned());
            match crate::agent_bridge::send_confirm_request(&token) {
                Ok(response) => send_agent_response(inner, response),
                Err(err) => send_agent_chat(inner, format!("[Agent] failed:\n{err}")),
            }
        });
    }

    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn spawn_agent_cancel(&mut self, id_or_token: String) {
        self.spawn_agent_cancel_parts(Some(id_or_token.clone()), Some(id_or_token), true);
    }

    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn spawn_agent_cancel_parts(
        &mut self,
        ...
        std::thread::spawn(move || {
            ...
            let token = token
                .map(|value| value.trim().to_owned())
                .filter(|value| !value.is_empty());
            if request_id.is_none() && token.is_none() {
                let response = crate::agent_bridge::AgentResponse {
                    request_id: String::new(),
                    project: String::new(),
                    status: "failed".to_owned(),
                    text: "Usage: /agent-cancel <request_id_or_token>".to_owned(),
                    token: None,
                    detail_json: None,
                };
                if chat_compat {
                    send_agent_chat(inner, format!("[Agent] failed:\n{}", response.text));
                } else {
                    send_agent_protocol_response(inner, response);
                }
                return;
            }
            match crate::agent_bridge::send_cancel_request_parts(
                request_id.as_deref(),
                token.as_deref(),
            ) {
                Ok(_) => {
                    let id = request_id.or(token).unwrap_or_default();
                    let response = crate::agent_bridge::AgentResponse {
                        request_id: id.clone(),
                        project: String::new(),
                    ...
                    if chat_compat {
                        send_agent_chat(inner, format!("[Agent] cancelled: {id}"));
                    } else {
                        send_agent_protocol_response(inner, response);
                    }
                }
                Err(err) => {
                    let response = crate::agent_bridge::AgentResponse {
                        request_id: request_id.or(token).unwrap_or_default(),
                        project: String::new(),
                        status: "failed".to_owned(),
                        text: err.to_string(),
                        token: None,
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn spawn_agent_status(&mut self, request_id: String, chat_compat: bool) {
        let inner = self.inner.clone();
        std::thread::spawn(move || {
            ...
            if request_id.is_empty() {
                let response = crate::agent_bridge::AgentResponse {
                    ...
                    project: String::new(),
                    status: "failed".to_owned(),
                    text: "Missing request_id".to_owned(),
                    token: None,
                    detail_json: None,
                };
                if chat_compat {
                    send_agent_chat(inner, "[Agent] failed:\nMissing request_id".to_owned());
                } else {
                    send_agent_protocol_response(inner, response);
                }
                return;
            }
            match crate::agent_bridge::send_task_status_request(&request_id) {
                Ok(task) => {
                    let detail_json = if task.detail_json.is_empty() {
                        None
                    } else {
                        Some(task.detail_json.clone())
                    };
                    let response = crate::agent_bridge::AgentResponse {
                    ...
                    if chat_compat {
                        ...
                        send_agent_protocol_response_with_task(inner, response, Some(&task));
                    }
                }
                Err(err) => {
                    let response = crate::agent_bridge::AgentResponse {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn handle_agent_command(&mut self, command: AgentCommand) {
        let mode = command
            .mode
            .enum_value_or(AgentCommandMode::AGENT_COMMAND_MODE_READ_ONLY);
        let action = serde_json::from_str::<Value>(command.prompt.trim())
            .ok()
            .and_then(|value| {
                value
                    .get("action")
            ...
            .unwrap_or_default();
        log::info!(
            "#{} agent command received request={} project={} mode={:?} action={} prompt_len={}",
            self.inner.id(),
            command.request_id,
            command.project,
            mode,
        ...
        match mode {
            AgentCommandMode::AGENT_COMMAND_MODE_STATUS => {
                self.spawn_agent_status(command.request_id, false);
            }
            AgentCommandMode::AGENT_COMMAND_MODE_CANCEL => {
                self.spawn_agent_cancel_parts(Some(command.request_id), None, false);
            }
            AgentCommandMode::AGENT_COMMAND_MODE_WORKSPACE_WRITE => {
                let request_id = if command.request_id.trim().is_empty() {
                ...
                self.spawn_agent_run(
                    ...
                    Some(command.require_confirmation),
                    None,
                    None,
                    None,
                    None,
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn handle_agent_cancel(&mut self, cancel: AgentCancel) {
        self.spawn_agent_cancel_parts(Some(cancel.request_id), Some(cancel.token), false);
    }
            ...
            match msg.union {
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::ChatMessage(c)) => {
                        #[cfg(not(any(target_os = "android", target_os = "ios")))]
                        if self.try_handle_agent_chat(&c.text) {
                    ...
                    #[cfg(not(any(target_os = "android", target_os = "ios")))]
                    Some(misc::Union::AgentCommand(command)) => {
                        self.handle_agent_command(command);
                        self.chat_unanswered = true;
                        self.update_auto_disconnect_timer();
                    }
                    #[cfg(not(any(target_os = "android", target_os = "ios")))]
                    Some(misc::Union::AgentCancel(cancel)) => {
                        self.handle_agent_cancel(cancel);
                        self.chat_unanswered = true;
                        self.update_auto_disconnect_timer();
                    }
...
mod test {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    #[test]
    fn task_snapshot_detail_json_for_response_uses_task_override() {
        let response = crate::agent_bridge::AgentResponse {
        ...
        let task = crate::agent_bridge::AgentTaskInfo {
            request_id: "req-1".to_owned(),
            project: "rustdesk".to_owned(),
            status: "done".to_owned(),
            text: "ok".to_owned(),
            ...
            error: String::new(),
            token: Some("token-1".to_owned()),
            detail_json: "{\"kind\":\"raw\"}".to_owned(),
            cancel_requested: false,
            timeline: vec![crate::agent_bridge::AgentTimelineEvent {
                stage: "done".to_owned(),
                summary: "completed".to_owned(),
                ts: 30,
                raw: None,
            }],
            raw_events: vec![serde_json::json!({"kind":"done"})],
        };

        let actual = task_snapshot_detail_json_for_response(&response, Some(&task));
        let expected = Some(crate::agent_bridge::task_snapshot_detail(&task).to_string());
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    #[test]
    fn task_snapshot_detail_json_for_response_skips_non_snapshot_status() {
        let response = crate::agent_bridge::AgentResponse {
