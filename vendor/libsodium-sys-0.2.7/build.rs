...
fn main() {
    ...
    let target_env_is_msvc = target.contains("msvc");
    let lib_dir_isset = env::var("SODIUM_LIB_DIR").is_ok() && !target_is_windows;
    ...
    if lib_dir_isset {
        find_libsodium_env(target_env_is_msvc);
...
*/
fn find_libsodium_env(target_env_is_msvc: bool) {
    ...
    let name = if target_env_is_msvc {
        "libsodium"
    } else {
        "sodium"
    };
...
fn make_libsodium(target: &str, source_dir: &Path, install_dir: &Path) -> PathBuf {
    ...
    if env::var("CARGO_CFG_TARGET_OS").ok().as_deref() == Some("windows") {
        // On native Windows builds we link against the bundled precompiled libs.
        return get_lib_dir();
    }
