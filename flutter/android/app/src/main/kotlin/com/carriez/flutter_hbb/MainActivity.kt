...
class MainActivity : FlutterActivity() {
    ...
    private fun initFlutterChannel(flutterMethodChannel: MethodChannel) {
        flutterMethodChannel.setMethodCallHandler { call, result ->
            ...
            when (call.method) {
                ...
                "record_agent_voice_clip" -> {
                    val durationMs = when (val args = call.arguments) {
                        is Map<*, *> -> (args["durationMs"] as? Number)?.toInt() ?: 5000
                        is Number -> args.toInt()
                        else -> 5000
                    }.coerceIn(1000, 15000)
                    thread {
                        try {
                            val audio = recordAgentVoiceClip(durationMs)
                            runOnUiThread {
                                result.success(Base64.encodeToString(audio, Base64.NO_WRAP))
                            }
                        } catch (e: Exception) {
                            Log.e(logTag, "record_agent_voice_clip failed", e)
                            runOnUiThread {
                                result.error("-1", e.message ?: "Failed to record voice clip", null)
                            }
                        }
                    }
                }
    ...
    private fun recordAgentVoiceClip(durationMs: Int): ByteArray {
        ...
        try {
            ...
            if (recorder.recordingState != AudioRecord.RECORDSTATE_RECORDING) {
                throw IllegalStateException("AudioRecord did not start recording")
            }
        ...
        if (pcmBytes.isEmpty()) {
            throw IllegalStateException("Recorded voice clip is empty")
        }
        return wavBytes(pcmBytes, sampleRate, 1, 16)
    }
