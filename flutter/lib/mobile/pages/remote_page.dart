...
class _RemotePageState extends State<RemotePage> with WidgetsBindingObserver {
  ...
  void onSoftKeyboardChanged(bool visible) {
    if (gFFI.chatModel.isAgentDashboardOverlayVisible) {
      if (visible) {
        _iosKeyboardWorkaroundTimer?.cancel();
        ...
        _timer = Timer(kMobileDelaySoftKeyboardFocus, () {
          SystemChrome.setEnabledSystemUIMode(SystemUiMode.manual,
              overlays: SystemUiOverlay.values);
          gFFI.chatModel.requestAgentDashboardInputFocus(
            delay: Duration.zero,
            enableSoftKeyboard: false,
          );
        });
      } else if (gFFI.ffiModel.pi.version.isNotEmpty) {
    ...
    if (!visible) {
      ...
      if (gFFI.chatModel.chatWindowOverlayEntry == null &&
          !gFFI.chatModel.isAgentDashboardOverlayVisible &&
  ...
  onPressedTextChat(String id) {
    ...
    gFFI.chatModel.showAgentDashboardOverlay();
  }

  showChatOptions(String id) async {
    onPressVoiceCall() async {
      if (!await gFFI.chatModel.ensureVoiceCallPermission()) {
        return;
      }
