...
class TerminalModel with ChangeNotifier {
  ...
  final _recentTranscriptChunks = <String>[];
  int _recentTranscriptSize = 0;
  static const int _kMaxRecentTranscriptChars = 16 * 1024;
  ...
  void _appendRecentTranscript(String text) {
    if (text.isEmpty) return;
    if (text.length >= _kMaxRecentTranscriptChars) {
      final truncated = text.substring(text.length - _kMaxRecentTranscriptChars);
      _recentTranscriptChunks
        ..clear()
        ..add(truncated);
      _recentTranscriptSize = truncated.length;
      return;
    }
    _recentTranscriptChunks.add(text);
    _recentTranscriptSize += text.length;
    while (_recentTranscriptSize > _kMaxRecentTranscriptChars &&
        _recentTranscriptChunks.length > 1) {
      final removed = _recentTranscriptChunks.removeAt(0);
      _recentTranscriptSize -= removed.length;
    }
  }

  String getRecentTranscript({int maxChars = 8000}) {
    if (_recentTranscriptChunks.isEmpty) {
      return '';
    }
    final joined = _recentTranscriptChunks.join();
