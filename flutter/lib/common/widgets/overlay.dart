...
class DraggableAgentDashboardWindow extends StatelessWidget {
  const DraggableAgentDashboardWindow({
    super.key,
    this.position = Offset.zero,
    required this.width,
    required this.height,
  ...
  @override
  Widget build(BuildContext context) {
    if (draggablePositions.agentDashboardWindow.isInvalid()) {
      draggablePositions.agentDashboardWindow.update(position);
    }
    return Draggable(
      checkKeyboard: true,
      checkScreenSize: true,
      position: draggablePositions.agentDashboardWindow,
      width: width,
      height: height,
      onPositionChanged: chatModel.setAgentDashboardWindowPosition,
      builder: (context, onPanUpdate) {
        return Material(
          ...
          child: Container(
            ...
            child: AgentDashboardPage(
              model: chatModel.agentDashboardModel,
              presentation: AgentDashboardPresentation.floatingWindow,
              header: _AgentDashboardWindowHeader(
                chatModel: chatModel,
                onPanUpdate: onPanUpdate,
              ),
            ),
...
class AgentDashboardBubble extends StatelessWidget {
  const AgentDashboardBubble({
    super.key,
    required this.chatModel,
  });
  ...
  @override
  Widget build(BuildContext context) {
    final initial = chatModel.agentDashboardBubblePosition;
    if (draggablePositions.agentDashboardBubble.isInvalid()) {
      draggablePositions.agentDashboardBubble.update(initial);
    }
    return Draggable(
      checkKeyboard: false,
      checkScreenSize: true,
      position: draggablePositions.agentDashboardBubble,
      width: 64,
      height: 64,
      onPositionChanged: chatModel.setAgentDashboardBubblePosition,
      builder: (context, onPanUpdate) {
        return GestureDetector(
          onPanUpdate: onPanUpdate,
          onTap: () {
            chatModel.hideAgentDashboardBubbleOverlay();
            chatModel.showAgentDashboardOverlay();
          },
          child: Material(
            color: Colors.transparent,
            child: Container(
...
class _AgentDashboardWindowHeader extends StatelessWidget {
  const _AgentDashboardWindowHeader({
    required this.chatModel,
  ...
  @override
  Widget build(BuildContext context) {
    final dashboardModel = chatModel.agentDashboardModel;
    final conversation = dashboardModel.selectedConversation;
    final statusLabel = conversation == null
        ? 'idle'
        : dashboardModel.statusLabelForConversationObject(conversation);
    final size = MediaQuery.of(context).size;
    final dense = size.width < 520 && size.height >= size.width;
    return GestureDetector(
      onPanUpdate: onPanUpdate,
      child: Container(
        height: dense ? 44 : 52,
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              ...
              child: IconButton(
                tooltip: 'Minimize',
                onPressed: chatModel.minimizeAgentDashboardOverlay,
                icon: const Icon(
                  Icons.remove,
                  color: Colors.white,
                ),
            ...
            SizedBox(
              ...
              child: IconButton(
                tooltip: 'Close',
                onPressed: chatModel.hideAgentDashboardOverlay,
                icon: const Icon(
                  Icons.close,
                  color: Colors.white,
                ),
...
class DraggablePositions {
  ...
  static const kAgentDashboardWindow = 'draggablePositionAgentDashboard';
  static const kAgentDashboardBubble = 'draggablePositionAgentDashboardBubble';
  ...
  final agentDashboardWindow = DraggableKeyPosition(kAgentDashboardWindow);
  final agentDashboardBubble = DraggableKeyPosition(kAgentDashboardBubble);
  final mobileActions = DraggableKeyPosition(kMobileActions);
  final iOSDraggable = DraggableKeyPosition(kIOSDraggable);

  load() {
    chatWindow.load();
    agentDashboardWindow.load();
    agentDashboardBubble.load();
    mobileActions.load();
    iOSDraggable.load();
  }
}
