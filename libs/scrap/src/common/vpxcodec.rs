...
extern "C" {
    ...
    ) -> *mut vpx_codec_enc_cfg_t;
    fn scrap_vpx_enc_cfg_free(cfg: *mut vpx_codec_enc_cfg_t);
    fn scrap_vpx_enc_cfg_update_quality(
        cfg: *mut vpx_codec_enc_cfg_t,
        q_min: c_uint,
        q_max: c_uint,
        bitrate: c_uint,
    ) -> vpx_codec_err_t;
    fn scrap_vpx_enc_cfg_bitrate(cfg: *const vpx_codec_enc_cfg_t) -> c_uint;
    fn scrap_vpx_dec_cfg_new(threads: c_uint) -> *mut vpx_codec_dec_cfg_t;
    fn scrap_vpx_dec_cfg_free(cfg: *mut vpx_codec_dec_cfg_t);
}

fn vpx_failed_call(result: vpx_codec_err_t, line: u32, column: u32) -> Error {
...
impl EncoderApi for VpxEncoder {
        ...
        Self: Sized,
    {
        match cfg {
            crate::codec::EncoderCfg::VPX(config) => {
                ...
                let c = call_vpx_ptr!(scrap_vpx_enc_cfg_new(
                    i,
                    config.width,
                    config.height,
                    ...
                    (i444 && config.codec == VpxVideoCodecId::VP9) as c_int,
                ));
                ...
                */
                ...
                if init_result != VPX_CODEC_OK {
                    unsafe { scrap_vpx_enc_cfg_free(c) };
                    return Err(vpx_failed_call(init_result, line!(), column!()).into());
                }

                let controls_result: ResultType<()> = (|| {
                ...
                if let Err(err) = controls_result {
                    unsafe {
                        vpx_codec_destroy(&mut ctx);
                        scrap_vpx_enc_cfg_free(c);
                    }
                    return Err(err);
                }
    ...
    fn set_quality(&mut self, ratio: f32) -> ResultType<()> {
        ...
        call_vpx!(scrap_vpx_enc_cfg_update_quality(
    ...
    fn bitrate(&self) -> u32 {
        unsafe { scrap_vpx_enc_cfg_bitrate(self.cfg) }
    }
...
impl Drop for VpxEncoder {
    fn drop(&mut self) {
        unsafe {
            ...
            scrap_vpx_enc_cfg_free(self.cfg);
...
impl VpxDecoder {
    ...
    pub fn new(config: VpxDecoderConfig) -> Result<Self> {
        ...
        let mut ctx = Default::default();
        let cfg = call_vpx_ptr!(scrap_vpx_dec_cfg_new(codec_thread_num(64) as c_uint));
        /*
        unsafe {
        ...
        */
        ...
        unsafe { scrap_vpx_dec_cfg_free(cfg) };
        if init_result != VPX_CODEC_OK {
            return Err(vpx_failed_call(init_result, line!(), column!()));
        }
        Ok(Self { ctx })
    }
