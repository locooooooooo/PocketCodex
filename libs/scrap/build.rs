...
fn generate_bindings(
    ...
    let mut b = bindgen::builder()
        ...
        .allowlist_type("(__)?u?int(8|16|32|64)_t")
        .allowlist_type("u?int(8|16|32|64)_t")
...
fn compile_codec_config_shim(include_paths: &[PathBuf]) {
    let src_dir = env::var_os("CARGO_MANIFEST_DIR").unwrap();
    let src_dir = Path::new(&src_dir);
    let shim = src_dir
        .join("src")
        .join("bindings")
        .join("codec_config_shim.c");
    ...
    build.compile("codec_config_shim");
}
...
fn main() {
    ...
    find_package("libyuv");
    let vpx_includes = find_package("libvpx");
    let aom_includes = find_package("aom");
    gen_vcpkg_package("libvpx", "vpx_ffi.h", "vpx_ffi.rs", "^[vV].*");
    gen_vcpkg_package("aom", "aom_ffi.h", "aom_ffi.rs", "^(aom|AOM|OBU|AV1).*");
    gen_vcpkg_package("libyuv", "yuv_ffi.h", "yuv_ffi.rs", ".*");
    let mut codec_config_includes = vpx_includes;
    codec_config_includes.extend(aom_includes);
    compile_codec_config_shim(&codec_config_includes);
    // ffmpeg();
